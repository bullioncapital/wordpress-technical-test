class JSON_Editor extends HTMLElement {
  ...
  set_caret_from_pointer(pointer) {
    ...
    for (i = 0; i < nodes_to_explore.length; i++) {
      ...
      fount_at = this.get_position_of_occurrence(
        node.textContent,
        pointer.character,
        occurrence,
      );
      if (fount_at >= 0) break;
      occurrence -= this.get_number_of_occurrences(
        node.textContent,
        pointer.character,
      );
    }
  ...
  get_position_of_occurrence(string, sub_string, occurrence) {
    const position = string
      .split(sub_string, occurrence)
      .join(sub_string).length;
  ...
  get_number_of_occurrences(string, sub_string) {
    return sub_string
      ? string.replace(
          new RegExp(`[^${this.escape_regex_string(sub_string)}]`, "g"),
          "",
        ).length
      : 0;
  }
  ...
  get_text_nodes(element) {
    ...
    while ((node = walk.nextNode())) list.push(node);
    return list;
  }
  ...
  format_object(input, offset = 0) {
    ...
    if (input === null) return '<span part="null">null</span>';
    let output = "";
    ...
    output += Object.keys(input)
      .map((key, index, list) => {
        return `${"&nbsp;".repeat(
          offset + this.indent,
        )}<span part="key" part="key"><span part="key_quotes">\"</span>${key}<span part="key_quotes">\"</span></span><span part="colon">:</span><span part="value">${this.format_input(
          input[key],
          offset + this.indent,
        )}</span>${
          index < list.length - 1 ? '<span part="comma">,</span>' : ""
        }<br>\n`;
      })
      .join("");
  ...
  format_array(input, offset = 0) {
    ...
    output += input
      .map((value, index, list) => {
        return `${"&nbsp;".repeat(
          offset + this.indent,
        )}<span>${this.format_input(value, offset + this.indent)}</span>${
          index < list.length - 1 ? '<span part="comma">,</span>' : ""
        }<br>\n`;
      })
      .join("");
  ...
  format() {
    ...
    try {
      ...
      content =
        editor.innerText && JSON.parse(editor.innerText.split("\xa0").join(""));
    } catch (exception) {
      return;
    }
    ...
    if (!content || current_string_content == this.last_string_content) return;

    editor.innerHTML = this.format_input(content);
    this.last_string_content = current_string_content;
    if (pointer && focus) this.set_caret_from_pointer(pointer);
  }
